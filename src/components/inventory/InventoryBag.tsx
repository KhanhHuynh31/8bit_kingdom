      ...
      >
        <div
          style={{
            ...
            width: 8,
            height: 8,
            borderTop: `1.5px solid ${C.amber}`,
            borderLeft: `1.5px solid ${C.amber}`,
          }}
        />
        <div
          style={{
            ...
            height: 8,
            borderTop: `1.5px solid ${C.amber}`,
            borderRight: `1.5px solid ${C.amber}`,
          }}
        />
        ...
        {owned.length === 0 ? (
          ...
          >
            ...
            <span style={{ fontSize: 9 }}>
              Thử cầu nguyện tại Cổng Triệu Hồi
            </span>
          </div>
        ) : (
          owned.map((deco) => (
    ...
    <div
      ...
      style={{
        ...
        boxShadow: isSelecting
          ? `0 0 12px ${RARITY_GLOW[deco.rarity]}`
          : "none",
      }}
    >
      <div
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div
          style={{
            fontSize: 9,
            color: RARITY_COLOR[deco.rarity],
            letterSpacing: 1,
            marginTop: 1,
          }}
        >
...
function PlacedDecoOnMap({
  ...
  const deco = useMemo(
    () => ALL_DECOS.find((d) => d.id === placed.decoId),
    [placed.decoId],
  );
...
export default function InventoryBag({
  ...
  const activeDeco = useMemo(() => {
    const id =
      placingId ??
      (movingId
        ? placedDecos.find((p) => p.instanceId === movingId)?.decoId
        : null);
  ...
  const selectedPlaced =
    placedDecos.find((p) => p.instanceId === selectedId) ?? null;
  ...
  if (selectedPlaced) {
    const st = TILE_SIZE * camera.zoom;
    const { x, y } = worldToScreen(
      selectedPlaced.worldX,
      selectedPlaced.worldY,
      camera,
      width / 2,
      height / 2,
    );
    ...
    <>
      ...
      {ghostPos && activeDeco && (placingId || movingId) && (
        <GhostPreview
          ...
          tileW={activeDeco.tileW}
          tileH={activeDeco.tileH}
          worldX={ghostPos.x}
          worldY={ghostPos.y}
          camera={camera}
          containerW={width}
      ...
      {placedDecos.map((p) => (
        <PlacedDecoOnMap
          ...
          onClick={(e) => {
            ...
            // Toggle popup
            setSelectedId((prev) =>
              prev === p.instanceId ? null : p.instanceId,
            );
          }}
        />
      ))}
      ...
      {selectedId && selectedPlaced && (
        <DeletePopup
          name={
            ALL_DECOS.find((d) => d.id === selectedPlaced.decoId)?.name ??
            selectedPlaced.decoId
          }
