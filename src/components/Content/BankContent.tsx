    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="text-blue-400 font-mono">
                    ×{CONVERSION.SUB} = {(ytStats.subscribers * CONVERSION.SUB).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-blue-100/60">Lượt xem (View)</span>
                  <span className="text-blue-400 font-mono">
                    ×{CONVERSION.VIEW} = {(ytStats.views * CONVERSION.VIEW).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-yellow-100/60">Lượt thích (Like)</span>
                  <span className="text-yellow-400 font-mono">
                    ×{CONVERSION.LIKE} = {(ytStats.totalLikes * CONVERSION.LIKE).toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-purple-100/60">Bình luận</span>
                  <span className="text-purple-400 font-mono">
                    ×{CONVERSION.COMMENT} = {(ytStats.totalComments * CONVERSION.COMMENT).toLocaleString()}
                  </span>
                </div>
                <div className="pt-2 border-t border-blue-900/30 flex justify-between items-center">
                  <span className="text-[10px] text-white/40 italic">Tổng cộng</span>
          ...
          <div className="mt-3 flex items-center justify-between">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); fetchYtStats(true); }}
              disabled={isLoading}
        ...
        >
          ...
          <div className="text-4xl font-bold text-red-100 flex items-baseline gap-2">
            0 <span className="text-xs font-normal text-red-500 opacity-60">VNĐ</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-1 gap-2">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm text-blue-100 font-medium">Đăng ký kênh</div>
                  <div className="text-[10px] text-blue-600">Góp sức xây dựng vương quốc</div>
                </div>
              </div>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-sm text-red-100 font-medium">Tương tác video</div>
                  <div className="text-[10px] text-red-600">Mỗi Like là một nguồn động lực</div>
                </div>
              </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <div className="text-xs text-amber-200 font-bold uppercase tracking-widest">Mở mã QR Quyên Góp</div>
        ...
        <div className="overflow-hidden border border-[#3a2810] rounded bg-black/40 min-h-[100px] flex flex-col justify-center">
          {transactions.length > 0 ? (
            transactions.map((t, i) => (
              <div key={i} className="flex items-center justify-between p-4 border-b border-[#2a1a08] last:border-0 hover:bg-amber-900/10 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]" />
                  <div>
                    <div className="text-xs text-amber-200 font-medium">{t.user}</div>
                    <div className="text-[10px] italic text-amber-800 leading-relaxed">&quot;{t.msg}&quot;</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-emerald-400 font-mono font-bold">{t.amount}</div>
                  <div className="text-[8px] text-[#3a2810] uppercase font-semibold">{t.time}</div>
                </div>
              </div>
            ))
          ) : (
            <div className="p-8 text-center flex flex-col items-center gap-2">
              ...
              <p className="text-xs text-amber-900/60 font-medium">Sổ cái còn trống...</p>
              <p className="text-[10px] text-amber-700/40 uppercase tracking-wider">Hãy trở thành người đầu tiên donate cho vương quốc Bơ</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-6 text-center border border-dashed border-[#4a3410] rounded relative bg-[#140e06]">
        <p className="text-sm italic text-[#8a6840] max-w-md mx-auto leading-relaxed">
          &quot;Vương quốc không xây dựng từ gạch đá, mà từ lòng tin. Mỗi hành động của bạn giúp Cụ Bơ có thêm động lực.&quot;
        </p>
        <div className="mt-4 flex justify-center gap-6 text-[9px] uppercase tracking-widest text-[#4a3410] font-bold">
          <span className="flex items-center gap-1"><HandHeart className="w-3 h-3" /> Minh bạch hoàng gia</span>
          <span className="flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Bảo mật tuyệt đối</span>
        </div>
      </div>
      ...
      {showQrModal && (
        ...
        >
          ...
          >
            <button onClick={() => setShowQrModal(false)} className="absolute -top-3 -right-3 w-10 h-10 bg-amber-600 text-black rounded-full flex items-center justify-center hover:bg-amber-400 transition-colors shadow-lg border-2 border-[#1a1208]">
              <X className="w-6 h-6" />
            </button>

            <div className="text-center space-y-6">
              <div className="space-y-1">
                <div className="text-amber-500 uppercase text-[10px] tracking-[0.3em] font-black">Cống phẩm hoàng gia</div>
              ...
              <div className="aspect-square w-full bg-white rounded-xl p-3 shadow-inner flex items-center justify-center group overflow-hidden">
                Ảnh QR sẽ được hiển thị ở đây (placeholder)
              </div>

              <div className="space-y-2">
                <p className="text-amber-200 text-sm font-bold tracking-widest">NGÂN HÀNG MB BANK</p>
                <p className="text-amber-600 font-mono text-xl font-black">0123 4567 8999</p>
                <div className="pt-4 text-[10px] text-amber-900/60 leading-relaxed border-t border-amber-900/20 italic">
                  * Sau khi quyên góp, tên của bạn sẽ được <br/> khắc ghi vĩnh viễn trên sổ cái này.
                </div>
              </div>
            </div>
          </div>
