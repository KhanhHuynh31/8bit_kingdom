...
export const CinemaContent = ({ data }: { data: Building }) => {
  const [selectedCat, setSelectedCat] = useState<"all" | "video" | "short">(
    "all",
  );
      ...
      {activeId && (
        ...
        >
          ...
          >
            ...
            {/* Close button */}
            <button
              className="absolute top-2 right-2 z-10 w-6 h-6 flex items-center justify-center transition-all duration-200 hover:bg-amber-900/30"
              style={{
                border: "1px solid #3a2810",
                borderRadius: "2px",
                color: "#6b4c1e",
