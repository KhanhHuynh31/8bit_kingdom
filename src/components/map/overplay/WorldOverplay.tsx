...
const TUNA_QUOTES_NORMAL = [
  "Chào đằng ấy! 🌊",
  "Tuna đang đói... 🥑",
  "Cá ngừ cũng biết buồn đó! 🐟",
];
const TUNA_QUOTES_EVOLVED = [
  "Cái ao này thật nhỏ bé.",
  "Hãy donate để mở rộng ao!",
];
...
export default function WorldOverlay({
  ...
  const showBubble = latestNews
    ? String(lastReadId) !== String(latestNews.id)
    : false;

  const [popups, setPopups] = useState<Popup[]>([]);
  const [interactionMode, setInteractionMode] = useState<"menu" | "talking">(
    "menu",
  );
  ...
  const handleBuildingAction = useCallback(
    (building: Building) => {
  ...
  // ── Tương tác Tuna ──────────────────────────────────────────────────────
  const handleTunaInteract = useCallback(
    (type: "talk" | "feed") => {
      ...
      if (type === "talk") {
        ...
        setPopups((prev) => [
          ...
          {
            id: newId,
            x: TUNA_BUILDING.worldX,
            y: TUNA_BUILDING.worldY,
            label: "-100 🥑",
            color: "text-red-500",
          },
        ]);
  ...
  const tunaPos = useMemo(() => {
    if (!TUNA_BUILDING || !(tunaVisible || tunaAnimating || tunaDiving))
      return null;
    const coords = worldToScreen(
      TUNA_BUILDING.worldX,
      TUNA_BUILDING.worldY + tunaAnimOffsetY,
      camera,
      cX,
      cY,
    );
  ...
  return (
    <div className="absolute inset-0 pointer-events-none select-none">
      ...
      {tunaPos && tunaInfoOpen && !tunaDiving && (
        ...
        >
          <div
            className={`bg-[#0b1e2c]/95 backdrop-blur-md border-2 ${isEvolved ? "border-yellow-400 shadow-[0_0_20px_rgba(250,204,21,0.3)]" : "border-cyan-400/50"} rounded-2xl p-4 min-w-[200px]`}
          >
            <div className="flex justify-between items-center mb-2 border-b border-white/10 pb-2">
              <span className="text-cyan-300 text-[10px] font-bold uppercase">
                {isEvolved ? "Tuna Đại Đế" : "🐟 Tuna Nhỏ"}
              </span>
              <span className="text-[10px] text-white/50">
                {isEvolved ? "MAX LEVEL" : `${tunaProgress}/1000`}
              </span>
            </div>
            ...
            <div className="min-h-[50px] flex items-center justify-center">
              {interactionMode === "talking" ? (
                ...
                <div className="flex gap-2 w-full">
                  <button
                    onClick={() => handleTunaInteract("talk")}
                    className="flex-1 flex flex-col items-center p-2 bg-white/5 hover:bg-white/10 rounded-xl transition-all"
                  >
                    <MessageCircle size={18} className="text-cyan-400" />
                    <span className="text-[10px] mt-1 text-white">Chat</span>
                  </button>
                  {!isEvolved && (
                    <button
